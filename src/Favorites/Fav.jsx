...
class Fav extends Component {
  ...
  render() {
    ...
    return <Fragment>
      ...
      <h1 className="text-white display-3 my-4"><Link to={"/movies"}><BiIcons.BiArrowBack className="text-danger"/></Link> Favorites: </h1>
      <div className="row pt-5">
      {this.props.favorites.map((val)=>{
        let truthyVal = val.name || val.original_title || val.title;
        return <Card val = {val} name = {truthyVal} key ={val.id} className = {style.card} cat = {val.media_type}/>
      })}
      </div>
      </div>
    </Fragment>
